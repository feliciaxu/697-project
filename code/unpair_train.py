...
from datasets import Unpairdata, denorm
from models import Gene, Discri,Discriup
from torch.utils.tensorboard import SummaryWriter
from utils import AverageMeter, ProgressMeter


class Trainer(object):
    def __init__(self, train_loader, valid_loader, lr, epo, gen_dstd2ehcd_resume, gen_ehcd2dstd_resume, dis_dstd_resume, dis_ehcd_resume, spath, is_cuda):
        ...
        self.gen_dstd2ehcd = Gene()
        self.gen_ehcd2dstd = Gene()
        self.dis_dstd = Discri()
        self.dis_ehcd = Discri()
        ...
        with torch.no_grad():
            ...
            for batch_idx, (dstd_images, ehcd_images) in enumerate(self.valid_loader):
                ...
                if batch_idx == 0:
                    ...
                    save_image(
                        fake_ehcd_grid, f"{self.spath}/viz/fake_ehcd_{self.epoch}.png")
                    save_image(
                        fake_dstd_grid, f"{self.spath}/viz/fake_dstd_{self.epoch}.png")
                    save_image(
                        recn_ehcd_grid, f"{self.spath}/viz/recn_ehcd_{self.epoch}.png")
                    save_image(
                        recn_dstd_grid, f"{self.spath}/viz/recn_dstd_{self.epoch}.png")
    ...
    def save_model(self, model_type, model, model_content, is_best):
        mpath = f"{self.spath}/{self.epoch}_{model_type}.pth.tar"
        model_content["state_dict"] = model.state_dict()
        torch.save(model_content, mpath)
        print(f">>> Save '{model_type}' model to {mpath}")
        if is_best:
            best_path = f"{self.spath}/best_{model_type}.pth.tar"
            copyfile(mpath, best_path)

    def load_model(self, model_type, model, mpath, device):
        ckpt = torch.load(mpath, map_location=device)
        epoch = ckpt["epoch"]
        model.load_state_dict(ckpt["state_dict"])
        print(
            f">>> Load '{model_type}' model at epoch {epoch} from {mpath}")
...
if __name__ == "__main__":
    ...
    # Build data loaders
    train_set = Unpairdata(args.data, (256, 256), "train")
    valid_set = Unpairdata(args.data, (256, 256), "valid")
    ...
    # Create trainer
    trainer = Trainer(train_loader, valid_loader, args.lr, args.epo, args.gen_dstd2ehcd_resume,
                      args.gen_ehcd2dstd_resume, args.dis_dstd_resume, args.dis_ehcd_resume, args.spath, is_cuda)
    trainer.train()
