...
from datasets import Pairdata, denorm
from models import Discri, Ver1, Ver2, TotalGenLoss
...
class Trainer(object):
    def __init__(self, arch, train_loader, valid_loader, lr, epo, gen_resume, dis_resume, spath, is_cuda):
        ...
        self.epo = epo
        self.spath = spath
        os.makedirs(f"{self.spath}/viz", exist_ok=True)
        self.writer = SummaryWriter(log_dir=self.spath)

        self.is_cuda = is_cuda
        self.print_freq = 20
        self.best_gen_loss = 1e6

        self.gen = {"version1": Ver1, "version2": Ver2}[arch]()
        self.dis = Discri()
    ...
    def train(self):
        for e in range(self.start_epoch, self.epo):
            self.epoch = e
    ...
    def train_epoch(self):
        ...
        for batch_idx, (dstd_images, ehcd_images) in enumerate(self.train_loader):
            ...
            # -----
            Fim = self.gen(dstd_images)
            g_loss = self.dis_criterion(Fim, ehcd_images)

            # Total loss
            total_gen_loss = 0.2 * g_loss + 0.8 * \
                self.gen_criterion(ehcd_images, Fim)
    ...
    def validate(self):
        ...
        with torch.no_grad():
            ...
            for batch_idx, (dstd_images, ehcd_images) in enumerate(self.valid_loader):
                ...
                # Vis
                if batch_idx == 0:
                    fake_grid = denorm(make_grid(Fim.data)).div_(255.)
                    ehcd_grid = denorm(make_grid(ehcd_images.data)).div_(255.)
                    dstd_grid = denorm(make_grid(dstd_images.data)).div_(255.)
                    save_image(
                        fake_grid, f"{self.spath}/viz/fake_{self.epoch}.png")
                    save_image(
                        ehcd_grid, f"{self.spath}/viz/ehcd_{self.epoch}.png")
                    save_image(
                        dstd_grid, f"{self.spath}/viz/dstd_{self.epoch}.png")
    ...
    def save(self, loss):
        ...
        gen = f"{self.spath}/{self.epoch}_gen.pth.tar"
        dis = f"{self.spath}/{self.epoch}_dis.pth.tar"

        # Save generator and discriminator
        model_content["state_dict"] = self.gen.state_dict()
        torch.save(model_content, gen)
        print(f">>> Save generator to {gen}")
        model_content["state_dict"] = self.dis.state_dict()
        torch.save(model_content, dis)
        print(f">>> Save discriminator to {dis}")

        if is_best:
            copyfile(gen, f"{self.spath}/best_gen.pth.tar")
            copyfile(dis, f"{self.spath}/best_dis.pth.tar")
...
if __name__ == "__main__":
    ...
    vername = ["version1", "version2"]

    parser = argparse.ArgumentParser(description="PyTorch FUnIE-GAN Training")
    parser.add_argument("-d", "--data", default="../mix", type=str, metavar="PATH",
                        help="path to data (default: none)")
    parser.add_argument("-a", "--arch", metavar="ARCH", default="version2",
                        choices=vername,
    ...
    parser.add_argument("--epo", default=90, type=int, metavar="N",
                        help="number of total epo to run")
    ...
    parser.add_argument("--gen-resume", default="./result/59_gen.pth.tar", type=str, metavar="PATH",
                        help="path to latest generator checkpoint (default: none)")
    parser.add_argument("--dis-resume", default="./result/59_dis.pth.tar", type=str, metavar="PATH",
                        help="path to latest discriminator checkpoint (default: none)")
    parser.add_argument("--save-path", default="result", type=str, metavar="PATH",
                        help="path to save results (default: none)")

    args = parser.parse_args()

    # Build data loaders
    train_set = Pairdata(args.data, (256, 256), "train")
    valid_set = Pairdata(args.data, (256, 256), "valid")
    ...
    # Create trainer
    trainer = Trainer(args.arch, train_loader, valid_loader, args.lr, args.epo,
                      args.gen_resume, args.dis_resume, args.spath, is_cuda)
    trainer.train()
