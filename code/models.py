...
#this is the first version of GAN, kernel size is 3
class Ver1(nn.Module):
    def __init__(self, n_feats=32):
        super(Ver1, self).__init__()
...
#this is the second version of GAN, kernel size is 3 instead of 4
#We add a conv layer, now it's a six-layer model
class Ver2(nn.Module):
    def __init__(self, n_feats=32):
        super(Ver2, self).__init__()
        self.conv1 = build_conv_block(
    ...
    def forward(self, x):
        ...
        u1 = torch.cat([self.deconv1(d6), d5], dim=1)  
        u2 = torch.cat([self.deconv2(u1), d3], dim=1)  
        u3 = torch.cat([self.deconv3(u2), d1], dim=1)  
...
class Gene(nn.Module):
    ...
    def forward(self, x):
        ...
        a1 = self.relu(self.add_conv1(d5))  
...
# Build discriminator blocks
class Discriup(nn.Module):
    def __init__(self, n_feats=32):
        super(Discriup, self).__init__()
    ...
    def forward(self, x):
        ...
        x = self.block4(x)             
        x = self.block5(x)              
        valid = self.validility(x)      
        return valid.squeeze(1)

#generate different version models 
